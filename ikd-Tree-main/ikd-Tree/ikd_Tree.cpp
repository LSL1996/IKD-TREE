...
int KD_TREE<PointType>::size(){
    int s = 0;
    if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node){  // 该树不会重建
...
void KD_TREE<PointType>::start_thread(){
    ...
    pthread_create(&rebuild_thread, NULL, multi_thread_ptr, (void*) this); // pthread_create向线程函数传递参数 
...
void KD_TREE<PointType>::stop_thread(){
    ...
    if (rebuild_thread) 
		pthread_join(rebuild_thread, NULL);  // 函数pthread_join用来等待一个线程的结束,线程间同步的操作
	// 销毁线程
...
void * KD_TREE<PointType>::multi_thread_ptr(void * arg){
    ...
    handle->multi_thread_rebuild();  // 多线程重建
    return nullptr;    
}
...
void KD_TREE<PointType>::multi_thread_rebuild(){
    ...
    while (!terminated){
        ...
        if (Rebuild_Ptr != nullptr ){                    
            ...
            flatten(*Rebuild_Ptr, Rebuild_PCL_Storage, MULTI_THREAD_REC);   // 树的所有元素转成向量
...
int KD_TREE<PointType>::Add_Points(PointVector & PointToAdd, bool downsample_on){
    ...
    for (int i=0; i<PointToAdd.size();i++){
        if (downsample_switch){
            ...
            Box_of_Point.vertex_max[0] = Box_of_Point.vertex_min[0]+downsample_size;  // x 范围

            Box_of_Point.vertex_min[1] = floor(PointToAdd[i].y/downsample_size)*downsample_size;
            Box_of_Point.vertex_max[1] = Box_of_Point.vertex_min[1]+downsample_size;   // y 范围

            Box_of_Point.vertex_min[2] = floor(PointToAdd[i].z/downsample_size)*downsample_size;
            Box_of_Point.vertex_max[2] = Box_of_Point.vertex_min[2]+downsample_size;   // z 范围
            ...
            for (int index = 0; index < Downsample_Storage.size(); index++){
                ...
                if (tmp_dist < min_dist){
                    min_dist = tmp_dist;
                    downsample_result = Downsample_Storage[index];   //找到距离中心点最近的点
                }
            }
            ...
            if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node){  
                if (Downsample_Storage.size() > 1 || same_point(PointToAdd[i], downsample_result)){
                    if (Downsample_Storage.size() > 0) 
						Delete_by_range(&Root_Node, Box_of_Point, true, true);  // 将找到的点全部删除
                    Add_by_point(&Root_Node, downsample_result, true, Root_Node->division_axis);  // 添加最近的点
                    tmp_counter ++;                      
                }
            } else {
                if (Downsample_Storage.size() > 1 || same_point(PointToAdd[i], downsample_result)){
...
void KD_TREE<PointType>::Add_Point_Boxes(vector<BoxPointType> & BoxPoints){     
    for (int i=0;i < BoxPoints.size();i++){
        if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node){
            ...
            if (rebuild_flag){  // 如果需要重建树，就先存储该操作
                pthread_mutex_lock(&rebuild_logger_mutex_lock);
                Rebuild_Logger.push(operation);  //
                pthread_mutex_unlock(&rebuild_logger_mutex_lock);
            }               
...
void KD_TREE<PointType>::BuildTree(KD_TREE_NODE ** root, int l, int r, PointVector & Storage){
    ...
    *root = new KD_TREE_NODE;
    ...
    {
    case 0:
        nth_element(begin(Storage)+l, begin(Storage)+mid, begin(Storage)+r+1, point_cmp_x);  // nth_element实际上就是快速选择算法
        break;
    case 1:
...
int KD_TREE<PointType>::Delete_by_range(KD_TREE_NODE ** root,  BoxPointType boxpoint, bool allow_rebuild, bool is_downsample){   
    ...
    if ((Rebuild_Ptr == nullptr) || (*root)->left_son_ptr != *Rebuild_Ptr){
        tmp_counter += Delete_by_range(&((*root)->left_son_ptr), boxpoint, allow_rebuild, is_downsample);  // 递归
    } else {
        pthread_mutex_lock(&working_flag_mutex);
    ...
    if ((Rebuild_Ptr == nullptr) || (*root)->right_son_ptr != *Rebuild_Ptr){
        tmp_counter += Delete_by_range(&((*root)->right_son_ptr), boxpoint, allow_rebuild, is_downsample);  // 递归
    } else {
        pthread_mutex_lock(&working_flag_mutex);
...
void KD_TREE<PointType>::Add_by_range(KD_TREE_NODE ** root, BoxPointType boxpoint, bool allow_rebuild){
    ...
    if ((Rebuild_Ptr == nullptr) || (*root)->left_son_ptr != *Rebuild_Ptr){
        Add_by_range(&((*root)->left_son_ptr), boxpoint, allow_rebuild);  // 递归
    } else {
        pthread_mutex_lock(&working_flag_mutex);
        Add_by_range(&((*root)->left_son_ptr), boxpoint, false);  // 递归
        if (rebuild_flag){
            pthread_mutex_lock(&rebuild_logger_mutex_lock);
...
void KD_TREE<PointType>::Add_by_point(KD_TREE_NODE ** root, PointType point, bool allow_rebuild, int father_axis){     
    if (*root == nullptr){
        *root = new KD_TREE_NODE;
        InitTreeNode(*root);
        (*root)->point = point;
        (*root)->division_axis = (father_axis + 1) % 3;  //  x y z轮流
        Update(*root);
        return;
    }
    (*root)->working_flag = true;
...
void KD_TREE<PointType>::Search(KD_TREE_NODE * root, int k_nearest, PointType point, MANUAL_HEAP &q, double max_dist){
    ...
    int retval; 
	// pthread_mutex_trylock() 是 pthread_mutex_lock() 的非阻塞版本。如果 mutex 所引用的互斥对象当前被任何线程（包括当前线程）锁定，则将立即返回该调用
    ...
    if (!root->point_deleted){
        float dist = calc_dist(point, root->point);  //  判断了点距离根节点的距离
    ...
    if (q.size()< k_nearest || dist_left_node < q.top().dist && dist_right_node < q.top().dist){  // 
        if (dist_left_node <= dist_right_node) {
            if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != root->left_son_ptr){  // 重建树的考虑
...
void KD_TREE<PointType>::Search_by_radius(KD_TREE_NODE *root, PointType point, float radius, PointVector &Storage)
{
    ...
    if (dist > radius + sqrt(root->radius_sq)) return; // 不相交
    if (dist <= radius - sqrt(root->radius_sq))   // 包含整个树
    { 
        flatten(root, Storage, NOT_RECORD);
        return;
    }
    ...
    {
        pthread_mutex_lock(&search_flag_mutex);
        Search_by_radius(root->right_son_ptr, point, radius, Storage);      // 锁住再操作
        pthread_mutex_unlock(&search_flag_mutex);
    }    
    return;
}
...
void KD_TREE<PointType>::flatten(KD_TREE_NODE * root, PointVector &Storage, delete_point_storage_set storage_type){
    ...
    Push_Down(root);
    if (!root->point_deleted) {
        Storage.push_back(root->point);    // 每一个节点进行操作
    }
    switch (storage_type)
    {
    case NOT_RECORD:
